document.addEventListener('DOMContentLoaded', function(){
  ...
  function validateOrder(){ if(!start||!end) return; if(start.value && end.value && end.value<start.value){ end.setCustomValidity(''); } else { end.setCustomValidity(''); } }
  ...
  function renderSelected(){ if(!selected||!list) return; selected.innerHTML=''; list.querySelectorAll('input[type="checkbox"]').forEach(function(cb){ if(cb.checked){ var label=cb.closest('label'); var name=label?label.querySelector('.assignee-name')||label.querySelector('span:last-child'):null; var tag=document.createElement('span'); tag.className='tag'; tag.textContent=(name?name.textContent:''); var rm=document.createElement('span'); rm.className='rm'; rm.textContent='×'; rm.addEventListener('click', function(){ cb.checked=false; renderSelected(); }); tag.appendChild(rm); selected.appendChild(tag); } }); }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  var stepTips=window.taskCreateTips||[];
