...
<div id="cookie-consent" class="cookie-consent" aria-live="polite" aria-hidden="true" style="display:none;">
    <div class="cookie-consent__shell" role="dialog" aria-modal="true" aria-labelledby="cookie-title" aria-describedby="cookie-desc" tabindex="-1">
        <div class="cookie-consent__icon" aria-hidden="true">
            <svg viewBox="0 0 122.88 122.25" width="44" height="44">
                <g>
...
<style>
/* Bottom bar styles inspired by the requested design */
.cookie-consent {
    position: fixed;
    left: 0;
    right: 0;
...
.cookie-consent__shell {
    ...
    opacity: 0;
    transition: transform 0.28s cubic-bezier(.22,1,.36,1), opacity 0.22s ease, background-color 0.2s ease, border-color 0.2s ease;
}
.cookie-consent__shell.show {
...
html.dark-mode .cookie-consent__shell,
body.dark-mode .cookie-consent__shell {
    background: linear-gradient(180deg, rgba(13, 19, 35, 0.98) 0%, rgba(11, 17, 31, 0.96) 100%);
    color: #e5e7eb;
    border-color: rgba(148, 163, 184, 0.16);
}
html.dark-mode .cookie-consent__icon,
body.dark-mode .cookie-consent__icon {
...
html.dark-mode .cookie-desc a,
body.dark-mode .cookie-desc a,
html.dark-mode .cookie-link,
body.dark-mode .cookie-link {
    color: #c4b5fd;
}
html.dark-mode .accept-button,
body.dark-mode .accept-button {
    background: linear-gradient(180deg, #ffffff 0%, #f3f4f6 100%);
    color: #0f172a;
    box-shadow: 0 10px 22px rgba(0, 0, 0, 0.22);
}
html:not(.dark-mode) .cookie-consent__shell,
body:not(.dark-mode) .cookie-consent__shell {
    background: linear-gradient(180deg, rgba(255, 255, 255, 0.98) 0%, rgba(247, 249, 255, 0.98) 100%);
    color: #0f172a;
    border-color: rgba(15, 23, 42, 0.1);
}
html:not(.dark-mode) .cookie-consent__icon,
body:not(.dark-mode) .cookie-consent__icon {
    background: rgba(79, 70, 229, 0.08);
}
html:not(.dark-mode) .cookie-title,
body:not(.dark-mode) .cookie-title {
    color: #0f172a;
}
html:not(.dark-mode) .cookie-desc,
body:not(.dark-mode) .cookie-desc {
    color: #4b5563;
}
html:not(.dark-mode) .cookie-desc a,
body:not(.dark-mode) .cookie-desc a,
html:not(.dark-mode) .cookie-link,
body:not(.dark-mode) .cookie-link {
    color: #4f46e5;
}
html:not(.dark-mode) .accept-button,
body:not(.dark-mode) .accept-button {
    background: #ffffff;
    color: #4f46e5;
    border: 1px solid rgba(79, 70, 229, 0.22);
    box-shadow: 0 10px 22px rgba(79, 70, 229, 0.1);
}
html:not(.dark-mode) .accept-button:hover,
body:not(.dark-mode) .accept-button:hover {
...
(function(){
    ...
    function hasConsent() { try { return ['accepted', 'declined'].includes(localStorage.getItem(STORAGE_KEY)); } catch(e){ return document.cookie.includes('tm8_cc=1') || document.cookie.includes('tm8_cc=0'); } }
    ...
    function show(){ if(!el) return; el.style.display='flex'; el.setAttribute('aria-hidden','false'); try{ document.body.classList.add('cookie-open'); }catch(e){} const shell = el.querySelector('.cookie-consent__shell'); requestAnimationFrame(()=>{ if(shell){ shell.classList.add('show'); shell.focus(); } }); }
    function hide(){ if(!el) return; const shell = el.querySelector('.cookie-consent__shell'); if(shell){ shell.classList.remove('show'); } try{ document.body.classList.remove('cookie-open'); }catch(e){} setTimeout(()=>{ el.setAttribute('aria-hidden','true'); el.style.display='none'; },220); }
